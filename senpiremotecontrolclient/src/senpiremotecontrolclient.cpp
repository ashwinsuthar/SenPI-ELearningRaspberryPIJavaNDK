...
SenPiControlClient::SenPiControlClient(QObject *parent)
    : QObject(parent)
{
...
SenPiControlClient::~SenPiControlClient()
{
    saveSettings();
...
void SenPiControlClient::openNetworkSession()
{
...
bool SenPiControlClient::sendWakeOnLan()
{
...
void SenPiControlClient::updateLastConnections()
{
...
void SenPiControlClient::addLastConnection(QString hostName, QString password, int port)
{
...
void SenPiControlClient::clearLastConnections()
{
    lastConnectionList.clear();
}

void SenPiControlClient::initialize()
{
    tcpServer->close();
...
void SenPiControlClient::connectToHost()
{
...
void SenPiControlClient::connectToServer(int id)
{
...
void SenPiControlClient::startBroadcasting()
{
    initialize();
    sendBroadcast();
...
void SenPiControlClient::abortBroadcasting()
{
    broadcastTimer->stop();
}

void SenPiControlClient::abortConnectionRequest()
{
    connectionRequestTimer->stop();
}

void SenPiControlClient::saveSettings()
{
    QSettings settings("", "senpicontrol-client", this);
...
void SenPiControlClient::loadSettings()
{
    QSettings settings("", "senpicontrol-client", this);
...
void SenPiControlClient::sendConnectionRequest()
{
...
void SenPiControlClient::sendBroadcast()
{
...
void SenPiControlClient::newConnection()
{
    if (!tcpSocket) {
...
void SenPiControlClient::deleteConnection()
{
    tcpSocket->abort();
...
void SenPiControlClient::incomingData()
{
#ifdef QT_DEBUG
...
void SenPiControlClient::incomingUdpData()
{
...
void SenPiControlClient::addServer(QHostAddress hostAddress, QRCServerType serverType, bool connected)
{
    bool found = false;
...
void SenPiControlClient::initializeNetworkTimeoutTimer()
{
...
void SenPiControlClient::saveResolvedHostName(QHostInfo hostInfo)
{
    for (int i = 0; i < serverList.size(); i++)
    {
...
void SenPiControlClient::updateNetConfig()
{
...
#ifdef TRIAL
void SenPiControlClient::checkTrialExpiration()
{
    ...
    if (m_trialExpirationTime.isNull())
    {
        ...
        imei.remove("-");
        request     = new QNetworkRequest(QUrl("http://senpi.org/trail.php?imei=" + imei));
...
void SenPiControlClient::replyFinished(QNetworkReply *reply)
{
    QByteArray  data;
    QString     timeString;
    QDateTime   dateTime;
...
void SenPiControlClient::trialTimerTimeout()
{
    checkTrialExpiration();
}
#endif

void SenPiControlClient::clearServerList()
{
    serverList.clear();
    emit serversCleared();
}

void SenPiControlClient::incomingIcon(QByteArray data)
{
    QDataStream in(data);
...
void SenPiControlClient::incomingAmarokData(QByteArray data)
{
#ifdef QT_DEBUG
...
void SenPiControlClient::sendKey(quint32 key, quint32 modifiers,  bool keyPressed)
{   
    quint8 mode1 = 1;
...
void SenPiControlClient::sendButton(quint8 id, bool keyPressed)
{
    quint8 mode1 = 5;
...
void SenPiControlClient::sendKeyPress(quint8 id)
{
    sendButton(id, true);
}

void SenPiControlClient::sendKeyRelease(quint8 id)
{
   sendButton(id, false);
}

 void SenPiControlClient::sendMouseMove(int deltaX, int deltaY)
 {
 ...
 void SenPiControlClient::sendHorizontalScroll(int delta)
 {
     sendMouseScroll(1,delta);
 }

 void SenPiControlClient::sendVerticalScroll(int delta)
 {
     sendMouseScroll(2,delta);
 }

 void SenPiControlClient::sendMouseScroll(quint8 direction, qint8 delta)
 {
     if (delta == 0)
         return;
 ...
 void SenPiControlClient::sendMouseButton(quint8 button, bool buttonPressed)
 {
     quint8 mode1 = 2;
     quint8 mode2 = 1;
 ...
 void SenPiControlClient::sendMousePress(quint8 button)
 {
     sendMouseButton(button, true);
 }

 void SenPiControlClient::sendMouseRelease(quint8 button)
 {
     sendMouseButton(button, false);
 }

 void SenPiControlClient::sendControlClicked(bool down)
 {
     sendKey(Qt::Key_unknown, Qt::ControlModifier, down);
 }

 void SenPiControlClient::sendAltClicked(bool down)
 {
     sendKey(Qt::Key_unknown, Qt::AltModifier, down);
 }

 void SenPiControlClient::sendShiftClicked(bool down)
 {
     sendKey(Qt::Key_unknown, Qt::ShiftModifier, down);
 }

void SenPiControlClient::sendAction(int id, bool pressed)
{
    quint8 mode1 = 3;
...
void SenPiControlClient::sendLight(int code)
{
    quint8 mode1 = 4;
...
void SenPiControlClient::sendText(QString text)
{
    quint8 mode1 = 8;
...
void SenPiControlClient::sendKeepAlive()
{
    quint8 mode1 = 6;
...
void SenPiControlClient::sendVersion()
{
    quint8 mode1 = 7;
...
void SenPiControlClient::disconnect()
{
    deleteConnection();
}

bool SenPiControlClient::isConnected()
{
    if (tcpSocket == NULL)
    {
        return false;
