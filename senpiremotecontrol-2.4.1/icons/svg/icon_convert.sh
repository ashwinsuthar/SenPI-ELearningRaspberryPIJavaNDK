...
OUTPUT_PATH="../qremotecontrol.iconset/"
...
if [ $# -gt 0 ]; then
    ...
    do
        ...
        then
            ...
            do
                size=${sizes[$s]}
                rsvg-convert -w $size -h $size $svg_file -o "$OUTPUT_PATH""$svg_name"_"$size"x"$size"".png"
		size_new=`expr ${sizes[$s]} \* 2`
                rsvg-convert -w $size_new -h $size_new $svg_file -o "$OUTPUT_PATH""$svg_name"_"$size"x"$size""@2x.png"
