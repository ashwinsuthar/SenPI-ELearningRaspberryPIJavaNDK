...
void QGlobalFakeKey::sendKey(Qt::Key key, bool down)
{
    ...
    do {
        if (key >= Qt::Key_0 && key <= Qt::Key_9 ||
            ...
            keySym = QChar(key).toUpper().toAscii();
#else
            keySym = QChar(key).toUpper().toLatin1();
#endif
            break;
        }
...
#if (QT_VERSION < 0x050000)
            keySym = VkKeyScan(QChar(key).toAscii());
#else
            keySym = VkKeyScan(QChar(key).toLatin1());
#endif
            if (keySym & 0x0100) modifiers |= Qt::ShiftModifier;
            if (keySym & 0x0200) modifiers |= Qt::ControlModifier;
            if (keySym & 0x0400) modifiers |= Qt::AltModifier;
            break;
        }
    ...
    if (modifiers)
        sendKeyModifiers(modifiers,true);

    INPUT InputData={0};
    int iEvents;

    InputData.type = INPUT_KEYBOARD;
    InputData.ki.wVk = keySym;
    ...
    if (modifiers)
        sendKeyModifiers(modifiers,false);
...
void QGlobalFakeKey::sendKeyModifiers(Qt::KeyboardModifiers modifiers, bool down)
{
    if (modifiers & Qt::ShiftModifier)
        sendKey(modifierToKey(Qt::ShiftModifier), down);
    if (modifiers & Qt::ControlModifier)
        sendKey(modifierToKey(Qt::ControlModifier), down);
    if (modifiers & Qt::AltModifier)
        sendKey(modifierToKey(Qt::AltModifier), down);
    if (modifiers & Qt::MetaModifier)
        sendKey(modifierToKey(Qt::MetaModifier), down);
    if (modifiers & Qt::KeypadModifier)
        sendKey(modifierToKey(Qt::KeypadModifier), down);
}
