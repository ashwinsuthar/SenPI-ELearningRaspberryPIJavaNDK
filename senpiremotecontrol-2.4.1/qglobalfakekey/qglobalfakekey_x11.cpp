...
void QGlobalFakeKey::sendKey(Qt::Key key, bool down)
{
    ...
    // Search Keysym
    do {
        if (key >= Qt::Key_A && key <= Qt::Key_Z) // ASCII characters
        {
        ...
        if ((key < 0x1000 && key >= 0x20 )|| (key >= Qt::Key_0 && key <= Qt::Key_9) )
        {
            ...
            KeyCode keyCode = XKeysymToKeycode(display, keySym);    // Get native keycode
            int symsPerKeycode;
            KeySym *keymap =  XGetKeyboardMapping(display, keyCode, 1, &symsPerKeycode); // Get keyboard mapping
        #ifdef QT_DEBUG
            for(int i=0; i<symsPerKeycode;i++)
            {
                qDebug() << keySym << keymap[i]<<"keymap["<<i<<"]is:"<<XKeysymToString(keymap[i]);
            }
        #endif

            if (keySym == keymap[1] && key != keymap[0])
            {
                //shiftModifier
                sendKeyModifiers(Qt::ShiftModifier,true);
                shiftSet=true;
            }
            else if (keySym == keymap[4])
            {
                //altModifier
                sendKeyModifiers(Qt::AltModifier,true);
                altSet=true;
            }
    ...
    if (shiftSet)
        sendKeyModifiers(Qt::ShiftModifier,false);
    else if (altSet)
        sendKeyModifiers(Qt::AltModifier,false);
...
void QGlobalFakeKey::sendKeyModifiers(Qt::KeyboardModifiers modifiers, bool down)
{
    ...
    KeyCode shiftKeycode = XKeysymToKeycode(d->display, XK_Shift_L);
    KeyCode controlKeycode = XKeysymToKeycode(d->display, XK_Control_L);
    KeyCode altKeycode = XKeysymToKeycode(d->display, XK_Alt_L);
    KeyCode metaKeycode = XKeysymToKeycode(d->display, XK_Meta_L);
    KeyCode keypadKeycode = XKeysymToKeycode(d->display, XK_Num_Lock);

    if (modifiers & Qt::ShiftModifier)
        XTestFakeKeyEvent( d->display, shiftKeycode, down, 0);
    if (modifiers & Qt::ControlModifier)
        XTestFakeKeyEvent( d->display, controlKeycode, down, 0);
    if (modifiers & Qt::AltModifier)
        XTestFakeKeyEvent( d->display, altKeycode, down, 0);
    if (modifiers & Qt::MetaModifier)
        XTestFakeKeyEvent( d->display, metaKeycode, down, 0);
    if (modifiers & Qt::KeypadModifier)
        XTestFakeKeyEvent( d->display, keypadKeycode, down, 0);
...
void QGlobalFakeKey::sendButton(Qt::MouseButton button, bool down)
{
   ...
   XTestFakeButtonEvent(d->display, nativeButton, down, 0);
...
void QGlobalFakeKey::sendScroll(int direction, int delta, double acceleration)
{
    ...
    {
        XTestFakeButtonEvent(d->display, button, true, 0);
        XTestFakeButtonEvent(d->display, button, false, 0);
    }
